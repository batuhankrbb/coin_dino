import 'package:flutter/material.dart';

class CustomColor {
  static final shared = CustomColor._privateConstructor();

  CustomColor._privateConstructor();

  Color backgroundBlueColor = Color(0xFF4C66EB);
  Color backgroundDarkModeColor = Color(0xFF22252a);
  Color backgroundWhiteColor = Color(0xFFe5e5f0);
  Color lightGreen = Color(0xFF71C03E);
  Color darkGreen = Color(0xFF4CAF0B);
  Color white = Color(0xFFFFFFFF);
  Color black = Color(0xFF3B3B3B);
  Color darkGrey = Color(0xFF4C4C4C);
  Color lightGrey = Color(0xFF939393);
  Color textGrey = Color(0xFF545454);
  Color creamyWhite = Color(0xFFF9F9F7);
  Color red = Color(0xFFED505F);
  Color pink = Color(0xFFE65E8B);
  Color orange = Color(0xFFFF9802);
  Color lightOrange = Color(0xFFFFAA3F);
  Color darkerWhite = Color(0xFFFDFDFD);
}
